...
#define setClockPin 4
#define setReadPin 5
#define setWritePin 6
...
void configureSoftAP() {
  ...
  server.on("/", HTTP_GET, [](AsyncWebServerRequest *request){
    File config_html = LittleFS.open("/config.html");
    if (!config_html) {
  ...
  server.on("/save-config", HTTP_POST, [](AsyncWebServerRequest *request){
    ...
    auto extractParamValue = [request, &newConfig](const char *paramName, char *destination, size_t destinationSize) {
        if (request->hasParam(paramName, true)) {
            const AsyncWebParameter *p = request->getParam(paramName, true);
...
void setupWebServer() {
  ...
  server.on("/config", HTTP_GET, [](AsyncWebServerRequest *request){
    handleRoot(request,"/config.html");
  }).setAuthentication(config.adminPassword, config.adminPassword);
  ...
  server.on("/perso.css", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send(LittleFS, "/perso.css", "text/css");
  });
  ...
  server.on("/parametrer-temperature", HTTP_POST, [](AsyncWebServerRequest *request){
    if(request->hasParam("tempParametree", true)) {
        const AsyncWebParameter* p = request->getParam("tempParametree", true);
  ...
  server.on("/controler-pompe", HTTP_POST, [](AsyncWebServerRequest *request){
    ...
    if(request->hasParam("pompe", true)) {
      const AsyncWebParameter* p = request->getParam("pompe", true);
      Serial.println("Data received: " + p->value());
...
void setup() {
  ...
  Serial.begin(115200);
  sleep(5);
  if (debug) { Serial.println("Welcome to SPA - Balboa system"); }

  /* LittleFS monté en premier (config.html nécessaire en mode SoftAP) */
  ...
  setup_wifi();  // peut appeler configureSoftAP() qui enregistre les routes /
